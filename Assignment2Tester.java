...
{
    // These data members are the most common information in all tests we could write.
    ...
    // NOTE: It's super important to use stdOut to preserve System.out before hijacking
    ...
    // Every data member after this line is particular to the assignment being tested.
    // In order, you will see: ints that index each of the classes being tested,
    //      the names of those same classes
    //      Strings that represent the names of methods, or the output that methods produce
    private static final int        CAR = 0;
    private static final int        METER = 1;
    private static final int        TICKET = 2;
    private static final String[]   CLASSNAME_OF = {
        "Car",
        "ParkingMeter",
        "ParkingTicket"
    }; // end array
    ...
    // This kind of practice is admittedly dodgey -- if you ever create a project
    ...
    private static Object worryFreeNew(int classIndex, Object... initargs)
    {
        ...
        for (Constructor<?> c : CLASS_OF[classIndex].getDeclaredConstructors()) {
            try {
                ...
                // We want it to escape this process and halt whatever test is in progress.
            ...
            if (m.getName().equals(methodName))
                try {
                    Object result = m.invoke(
                        STANDARD_TEST_SUBJECT[classIndex], parameters);
                    if (forbidConsole) verifyNoOutput();
                    return result;
                         
                } catch (ReflectiveOperationException notTheRightSignature) {
                    // If all of these fail, then we'll exit the loop eventually
                    // and go through the last resort* at the end of this process.
                }
                 
        // * Last Resort: If the method couldn't be identified or run, then we Assert.fail().
        fail(String.format(
            "%s  %s.%s couldn't be used for some reason." + NEWLINE,
            "-1 mark.", CLASSNAME_OF[classIndex], lastReflection
        ));
         
        return null; // in theory, never happens -- since Assert.fail() is overbearing
    } // end method
    ...
    private Assignment2Tester process(Object... parameters)
    {
        try {
            ...
            System.err.println("The author of the testing module has made a horrible error!\nPlease inform them of this problem when you can.");
    ...
    private static Object getFieldValue(int classIndex, String fieldName)
    {
        ...
        try {
            ...
            fail(String.format(
                "%s  %s.%s does not exist." + NEWLINE,
                "-1 mark.", CLASSNAME_OF[classIndex], fieldName
            ));
        } // end try
    ...
    private static void verifyResult(Object expectedValue, Object actualValue, String failReason)
    {
        assertEquals(
            String.format(
                "%s mark.  %s.%s %s.%s",
                -1, lastClass.getName(), lastReflection, failReason, NEWLINE
            ),
            expectedValue,
            actualValue
        );
     ...
     */
    private static Field validateMember(int classIndex, String fieldName, Class<?> expectedType, int mandatoryModifiers)
    {
        lastClass = CLASS_OF[classIndex];
        lastReflection = fieldName;
        ...
        verifyResult(expectedType, field.getType(),
            "does not have the expected variable-type");
     ...
     */
    private static void validateConstant(int classIndex, String fieldName, Class<?> expectedType, Object expectedValue)
    {
        int settings = validateMember(classIndex, fieldName, expectedType).getModifiers();
         
        assertTrue(
            String.format(
                "%s  %s.%s is not private or public." + NEWLINE,
                "-1 mark.", CLASSNAME_OF[classIndex], fieldName
            ),
            Modifier.isPrivate(settings) || Modifier.isPublic(settings)
        );
    ...
    private void parseJavadoc(java.io.File file)
    {
        ...
        final String DEFAULT_CLASS_HEADER_PENALTY = "-1 mark.  The class header for '%s' has been left as the BlueJ-generated default." + NEWLINE;
        final String NO_CLASS_HEADER_PENALTY = "-1 mark.  The class header for '%s' is missing." + NEWLINE;
        final String UNDOCUMENTED_PARAM_PENALTY = "-1 mark.  Incorrect javadoc for parameter '%s', before %s.%s()." + NEWLINE;
        ...
        if (0 == text.size())
            fail(String.format(
                "You did not declare a class or interface in %s.",
                file.getName()));
        ...
        try {
            ...
            fail(String.format(
                "The tester is unable to assess %s." + NEWLINE
                    + "> This is OK is it's an interface or abstract class." + NEWLINE
                    + "> This is BAD if it's a class file." + NEWLINE,
                file.getName()
            ));
        } // end catch
        ...
        while (reader.hasNext())
        {
            ...
            text = scanIntoStack(reader,
                "((?<=\\*)\\s+@(param|return)\\s+\\S+)|(public[\\s0-z]* \\w+\\([^\\)]*\\))");
            ...
            /*if (0 != text.size())
                subtestResults += String.format("-1: %1s.%2s() has at least one @param or @return statement that is incorrect."
        ...
        while (reader.hasNext())
        {
            ...
            if (line.toUpperCase().contains("BONUS")) {
                stdOut.println(String.format(
                    "Line %s of %s contains the word \"bonus\":\n%s\n",
                    lineNumber, fileName, line
                ));
                 
                if (thisFileAppearsOnce) break;
    ...
    {
        stdOut.println("\u000c"); // clear the console window (may only work in Windows BlueJ)
    ...
    {
        subtest("validateField")
            .process(CAR, LICENSE, String.class)
            .process(CAR, TRANSMISSION, char.class)
            .process(CAR, ODOMETER, double.class)
            .process(CAR, PARK_DURATION, int.class)
            .process(METER, PARK_PAID, int.class);
    } // end test
 
    public void allConstantsExist()
    {
        subtest("validateConstant")
            .process(CAR, "MINIMUM_PLATE_LENGTH", int.class, 2)
            .process(CAR, "MAXIMUM_PLATE_LENGTH", int.class, 7)
            .process(CAR, "MANUAL_TRANSMISSION", char.class, 'm')
            .process(CAR, "AUTOMATIC_TRANSMISSION", char.class, 'a');
    } // end test
 
    public void carConstructorSetsDefaults()
    {
        subtest("verifyFieldValue")
            .process(CAR, TRANSMISSION, 'a')
            .process(CAR, ODOMETER, 50000.5)
            .process(CAR, PARK_DURATION, 0);
    } // end test
    ...
    public void meterAddMinutesPaidWorks()
    {
        final int[] inputs = new int[] {
            1 + (new java.util.Random()).nextInt(999), -1, 15, 0
        }; // end array
         
        final int[] outputs = new int[] {
            15 + inputs[3], 15, 15, 0
        }; // end array
        ...
        for (
            ...
            i != 0;
            runMethod(METER, "addMinutesPaid", inputs[--i])
        )
    ...
    {
        verifyResult(null, runMethod(
            TICKET, "checkParking",
            STANDARD_TEST_SUBJECT[CAR], STANDARD_TEST_SUBJECT[METER]));
    } // end test
    ...
    public void ticketReferenceChecksOut13Times()
    {
        ...
        } // end try: ensure that standard subjects are restored even when test fails
    } // end test
    ...
    public void ticketDetailsLessThanAnHourInfraction()
    {
        ...
        // via a hidden parameter (value of minutesParked in STANDARD_TEST_SUBJECT).
    ...
    public void ticketDetailsExactlyOneHourInfraction()
    {
        ...
        try {
            ...
            verifyOutput(String.format(
                ...
                RANDOM_REFERENCE, NEWLINE,
                RANDOM_LICENSE, NEWLINE,
                RANDOM_PARK, RANDOM_PAID, NEWLINE,
    ...
    {
        for (String a1TestName : new String[] {
            "allFieldsExist",
            "allConstantsExist",
            "carConstructorSetsDefaults",
            "carConstructorAcceptsGoodData",
            "carConstructorAcceptsNullLicense",
            "carConstructorSetsDefaults",
            "carConstructorRejectsBadLicense",
            "carLicenseAcceptsGoodData",
            "carLicenseAcceptsNull",
            "carLicenseRejectsBadData",
            "carTransmissionAcceptsGoodData",
            "carTransmissionRejectsBadData",
            "carOdometerReads50000Point5ByDefault",
            "carOdometerReads48151Point6WhenChanged",
            "carIsNotUsuallyParked",
            "carIsParkedWhenDurationNotZero",
            "carParkAcceptsGoodData",
            ...
            if (0 == subtestResults.length())
                ...
                subtestResults = "-2 marks.  Assignment1Tester."
                    + a1TestName + "() failed unexpectedly!" + NEWLINE + NEWLINE
                    + subtestResults.replaceAll("-\\d marks?.  ", "");
                fail();
            }
        ...
        setUp();
        assertEquals("-2 marks.  Assignment1Tester.meterMinutesPaidWorks() did not produce the expected failure message." + NEWLINE,
            "ParkingMeter.setMinutesPaid() couldn't be used for some reason." + NEWLINE + NEWLINE,
            expectedFail);
    } // end supertest
    ...
    {
        for (Object[] parameterSet : new Object[][] {
            new Object[] {DEFAULT_LICENSE, 0, 0},
    ...
    // inner classes should always appear at the end of the class that contains them:
     
    /**
     * This is a tiny mechanism that qualifies the files that are given to it
     * during Files.walkFileTree().  The Javadoc Test and Bonus Seeker want
     * to only consider files with the .java extension, found within the working directory.
     */
    private class ProjectFileVisitor
    extends
        java.nio.file.SimpleFileVisitor<java.nio.file.Path>
    {
        ...
        public java.nio.file.FileVisitResult visitFile(
            ...
            if (file.toString().endsWith(".java")) process(file.toFile());
